...
```sh
agent-session github-app run ... -- COMMAND [ARG]...
```

Prefer `agent-session github-app run ... -- COMMAND` for normal agent GitHub work
...
When changing release workflows, keep `x86_64-unknown-linux-musl`,
`aarch64-unknown-linux-musl`, and `aarch64-apple-darwin` assets working unless
