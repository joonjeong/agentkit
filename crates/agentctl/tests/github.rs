...
fn shows_top_level_help() {
    ...
    cmd.arg("--help").assert().success().stdout(
        predicate::str::contains("github-app")
            .and(predicate::str::contains("agentctl"))
            .and(predicate::str::contains("service"))
            .and(predicate::str::contains("telegram")),
    );
}
...
#[test]
fn shows_github_app_usage() {
    let mut cmd = Command::cargo_bin("agentctl").expect("binary exists");

    cmd.arg("--help")
        .assert()
        .success()
        .stdout(predicate::str::contains("github-app").and(predicate::str::contains("agentctl")));

    let mut github = Command::cargo_bin("agentctl").expect("binary exists");
    github
...
#[cfg(unix)]
#[test]
fn agentctl_symlink_style_help_does_not_duplicate_subcommand_name() {
    let mut cmd = std::process::Command::new(assert_cmd::cargo::cargo_bin("agentctl"));
    cmd.arg0("agentctl");
    ...
    assert!(stdout.contains("Usage: agentctl <COMMAND>"));
    assert!(!stdout.contains("agentctl agentctl [OPTIONS]"));
}
...
fn agentctl_can_configure_child_only_git_credentials() {
    ...
    let output_dir = unique_temp_dir("agentctl-git-credentials-test");
...
#[test]
fn agentctl_requires_command_after_separator() {
    let mut cmd = Command::cargo_bin("agentctl").expect("binary exists");
...
fn agentd_response_server(
    ...
    let socket_dir = unique_temp_dir("agentctl-agentd-test");
